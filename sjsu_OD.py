...
class ObjectDetector:
    ...
    def process_video(self, video_path, output_frame_dir, answer, skip_frames, suffix, rotate=None):
        ...
        while cap.isOpened():
            ...
            if frame_counter % skip_frames == 0:
                raw_frame_path = os.path.join(output_frame_dir, f"{answer}_{frame_counter:04d}_{suffix}.jpg")
                cv2.imwrite(raw_frame_path, frame_resized)
                ...
                frame_data = self.process_detected_objects(detected_objects, raw_frame_path, answer)
                ...
                # Generate different noise variations and process them
                noise_types = ["gaussian", "salt_pepper", "poisson"]
                for noise in noise_types:
                    noisy_image = self.add_noise(frame_resized, noise)
                    noisy_frame_path = os.path.join(output_frame_dir, f"{answer}_{noise}_{frame_counter:04d}_{suffix}.jpg")
                    cv2.imwrite(noisy_frame_path, noisy_image)

                    # Convert noisy image to PIL format
                    pil_noisy = Image.fromarray(cv2.cvtColor(noisy_image, cv2.COLOR_BGR2RGB))

                    # Store noisy frame data with original bounding boxes, replacing `filename` with `raw_frame_path`
                    noisy_frame_data = [
                        {**d, 'raw_frame_path': noisy_frame_path} for d in frame_data  # Copy and update path
                    ]
